			...
			<div class="row">
				<div class="col-lg-12">
					<div class="breadcrumb__links">
						<a href="../index.jsp"><i class="fa fa-home"></i>Accueil</a> <span>Panier</span>
					</div>
				</div>
			</div>
		</div>
 					  ...
 					  %>
								...
								<tr>
									<td class="product-thumbnail"><img
										src="../images/<%=pz.getImage() %>" alt="Image"
										class="img-fluid-panier"></td>
									...
									<td>
										<div
											...
											<div class="input-group-prepend">
												<button type="button" class="btn btn-black decrease"
													data-id="<%=lcm.getId_lignecom() %>">-</button>
											</div>
											<input type="text"
												class="form-panier text-center quantity-amount"
												value="<%= lcm.getQte_lignecom() %>" placeholder=""
												aria-label="Example text with button addon"
												aria-describedby="button-addon1">
											<div class="input-group-append">
												<button class="btn btn-black increase" type="button"
													data-id="<%=lcm.getId_lignecom() %>">+</button>
											</div>
										</div> <!-- <div class="input-group mb-3 d-flex align-items-center quantity-container">
                              <div class="input-group-prepend">
                              ...
                              <div class="input-group-append">
                                <button class="btn btn-black increase" type="button" data-id="<%=lcm.getIdpiz_lignecom() %>">+</button>
									...
									<td><a
										href='../PanierController?idlc=<%=lcm.getId_lignecom()%>&supp=true'
							...
							<div class="row">
								<div class="col-md-12">
									...
									<form action="../PanierController" method="post">
										<input type="hidden" name="idcomach"
											value=<%=cm.getId_com()%>> <input type="hidden"
											name="idcliach" value=<%=c.getId()%>>
...
<script type="text/javascript">
	$(document)
			.ready(
					function() {
						$('.quantity-container .increase').on(
								'click',
								function() {
									var quantityAmount = $(this).closest(
											'.quantity-container').find(
											'.quantity-amount');
									var value = parseInt(quantityAmount.val(),
											10);
						...
						$('.quantity-container .decrease')
								.on(
										...
										function() {
											...
											var value = parseInt(quantityAmount
													.val(), 10);
											value = isNaN(value) ? 0 : value;
											if (value > 1) {
												value--;
						...
						function redirectToPanierControler(dataId, value) {
							window.location.href = '../PanierController?dataId='
									+ dataId + '&value=' + value;
						}
					});
